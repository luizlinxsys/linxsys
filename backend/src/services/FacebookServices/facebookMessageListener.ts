...
const verifyContact = async (msgContact: any) => {
  ...
  try {
    if (webhookEvent.message) {
      ...
      if (ticket.queue && ticket.queueId) {
        if (!ticket.user) {
          await sayChatbot(ticket.queueId, getSession, ticket, contact, message);
        }
      }
...
const verifyQueue = async (
  ...
  const { queues, greetingMessage } = await ShowWhatsAppService(getSession.id!);
